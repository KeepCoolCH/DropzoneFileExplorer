...
function safe_upload_filename(string $fileName): string {
  ...
  if ($fileName === '' || $fileName === '.' || $fileName === '..') {
    jsend(['ok'=>false,'error'=>'Invalid file name'], 400);
  }
  return $fileName;
}
...
function zip_create_job(array $absPaths, string $label, string $baseDir, array $extraExcludes = []): array
{
    ...
    $zipBin = is_executable('/usr/bin/zip') ? '/usr/bin/zip' : 'zip';
    $cmd = 'cd ' . escapeshellarg($baseDir) . ' && ' . escapeshellcmd($zipBin) . ' -r -0 ' .
...
if ($isApi) {
  ...
  $action = (string)($_GET['action'] ?? $_POST['action'] ?? '');
  $body = read_json_body();
  if (AUTH_ENABLE && $_SERVER['REQUEST_METHOD'] === 'POST') {
    $csrf = (string)($_SERVER['HTTP_X_CSRF_TOKEN'] ?? $_POST['csrf'] ?? $body['csrf'] ?? '');
    if (!csrf_check($csrf)) {
      jsend(['ok'=>false,'error'=>'Invalid session'], 403);
    }
  }
...
if ($action === 'zipCreate') {
  ...
  if ($rootReal === false) jsend(['ok'=>false,'error'=>'Root missing'], 500);
  $zipBaseRel = norm_rel(substr($zipBase, strlen(rtrim($rootReal, DIRECTORY_SEPARATOR)) + 1));
  ensure_write_allowed($zipBaseRel);
  if (!empty($body['name'])) {
  ...
  try {
    ...
    jsend(['ok'=>false,'error'=>'ZIP failed'], 500);
  }
  $finalPath = $zipBase . DIRECTORY_SEPARATOR . $res['name'];
  ensure_target_inside_allowed_roots($finalPath);
...
if ($action === 'unzip') {
  ...
  $destAbs = abs_path($dest);
  ensure_inside_allowed_roots($zipAbs);
  if (!is_file($zipAbs)) {
...
if ($action === 'uploadInit') {
  ...
  $fileName = safe_upload_filename((string)($body['fileName'] ?? 'file'));
...
if ($action === 'uploadChunk') {
  ...
  if ($uploadId === '' || $index < 0 || $total <= 0) {
    jsend(['ok'=>false,'error'=>'Bad params'], 400);
  }
...
if (!is_dir($udir)) jsend(['ok'=>false,'error'=>'Unknown uploadId'], 404);
  ...
  if (!$saved) {
    jsend(['ok'=>false,'error'=>'Save chunk failed'], 500);
  }
  $metaPath = $udir.'/meta.json';
  if (is_file($metaPath)) {
...
if ($action === 'uploadFinalize') {
  ...
  $fileName = safe_upload_filename((string)($meta['fileName'] ?? 'file'));
  ...
  if ($relPath !== '') {
    ...
    ensure_target_inside_allowed_roots($finalDirAbs);
    @mkdir($finalDirAbs, 0775, true);
  }
  $finalName = $fileName;
  $finalAbs = $finalDirAbs . DIRECTORY_SEPARATOR . $finalName;
  ensure_target_inside_allowed_roots($finalAbs);
